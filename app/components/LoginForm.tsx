...
export default function LoginForm() {
  ...
  // const [jsonData, setJsonData] = useState<SignedOffchainAttestation | null>(
  //   null
  // );
  ...
  return (
    <div>
        ...
        >
          ...
          <p className={`${jsonData ? "text-green-500" : "text-gray-500"}`}>
            {isDragActive
              ...
              : jsonData
              ? "the json file is accepted"
              : "Drag & drop a JSON file here, or click to select one"}
          </p>
          {!jsonData && (
            <button
              className={`mt-2 px-4 py-2 ${
                jsonData ? "bg-green-500" : "bg-blue-600"
              } text-white rounded shadow hover:bg-blue-500 focus:outline-none`}
            >
      ...
      <MetamaskConnect />
      {/* <button
        className="bg-blue-600 text-lg font-extrabold py-4 px-10 rounded-full hover:bg-black mt-5 w-full disabled:opacity-50"
